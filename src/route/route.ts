...
import { authorSchema, bookSchema, bookAuthorSchema } from '../schemas/schemas.js'
import {
  createAuthor,
  getAuthorById,
  getAllAuthors,
...
export const requestHandler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
  ...
  try {
    ...
    if (method === 'GET' && pathname && pathname.startsWith('/docs/')) {
      ...
      const file = pathname === '/docs/' ? 'index.html' : pathname.replace('/docs/', '')
    ...
    if (pathname === '/authors' && method === 'POST') {
      ...
      if (!parsed.success) return sendJson(res, 400, { error: 'invalid payload', issues: parsed.error.issues })
    ...
    // Book with authors
    if (pathname && pathname.startsWith('/books/') && pathname.endsWith('/authors') && method === 'GET') {
      const id = Number(pathname.split('/')[2])
    ...
    if (pathname === '/book-author' && method === 'POST') {
      ...
      if (!parsed.success) return sendJson(res, 400, { error: 'invalid payload', issues: parsed.error.issues })
      addAuthorToBook(parsed.data.firstName, parsed.data.lastName, parsed.data.title)
