...
const insertBookAuthor = (
  ...
  console.log(
    "cleanFirstName cleanLastName  cleanTitle",
    cleanFirstName,
    cleanLastName,
    cleanTitle,
  )
...
const getAuthorById = (id: number) => {
  return db
    .prepare("SELECT id, firstName, lastName FROM author WHERE id = ?")
    .get(id)
}
...
const getAllAuthors = () => {
  return db
    .prepare(
      "SELECT id, firstName, lastName FROM author ORDER BY lastName, firstName",
    )
    .all()
}
...
const updateAuthor = (id: number, firstName: string, lastName: string) => {
  ...
  const l = cleanString(validateString(lastName))
  return db
    .prepare("UPDATE author SET firstName = ?, lastName = ? WHERE id = ?")
    .run(f, l, id)
}
...
const createBook = (
  ...
  const stmt = db.prepare(
    "INSERT INTO book (title, genre, published, state) VALUES (?, ?, ?, ?)",
  )
...
const getBookById = (id: number) => {
  return db
    .prepare("SELECT id, title, genre, published, state FROM book WHERE id = ?")
    .get(id)
}
...
const getAllBooks = () => {
  return db
    .prepare(
      "SELECT id, title, genre, published, state FROM book ORDER BY title",
    )
    .all()
}
...
const updateBook = (
  ...
  return db
    .prepare(
      "UPDATE book SET title = ?, genre = ?, published = ?, state = ? WHERE id = ?",
    )
...
const addAuthorToBook = (
  ...
  const bookRow = db
    .prepare("SELECT id FROM book WHERE lower(title) = ?")
    .get(t)
  const authorRow = db
    .prepare(
      "SELECT id FROM author WHERE lower(firstName) = ? AND lower(lastName) = ?",
